...
class DDriveServiceCommand extends Command {
  static keyFlag (info) {
    return flags.build({
  ...
  parsePath (mnt, path) {
    ...
    path = p.resolve(path)
    if (path.startsWith(p.join(mnt, 'Network'))) throw new Error('DDrive commands cannot be run on the Network directory.')
  ...
  async run () {
    try {
      this.client = new DDriveServiceClient()
      await this.client.ready()
    } catch (err) {
      console.log('err:', err)
      console.error('Could not connect to the DDrive service. Is it running? Check with `ddrive status`.')
      this.exit(1)
    }
  }
}
...
module.exports = DDriveServiceCommand
